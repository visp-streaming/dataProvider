...
public class RandomWalkHalf extends GenerationPattern{

    public GenerationState iterate(GenerationState state) {

        HashMap<Integer, Double> states = new HashMap<>();
        states.put(1, 2.0);
        states.put(2, 2.0);
        states.put(3, 2.0);
        states.put(4, 3.0);
        ...
        if (state.getIteration() > 124) {
            Integer currentstate = 0;
            if (!state.getDirection().equals("up")) {
                currentstate = Integer.parseInt(state.getDirection());
            }

            currentstate++;

            if ((currentstate - 3) > states.size()) {
                currentstate = 1;
            }

            state.setAmount(states.get(currentstate).intValue());
            state.setDirection(currentstate.toString());
